...
func sendAppendEntries(rn *RaftNode, init bool, lastEntryIdx uint64, higherTermChan chan uint64) {
	rn.log.Print("updateCommitIndex")
	for nd, _ := range rn.node.PeerNodes {
		if nd == rn.node.ID {
			continue
		}
		rn.log.Printf("Leader send AppendEntries to %v, lastLogIdx %v,commitIdx %v, nextIdx %v, matchIdx %v", nd, lastEntryIdx, rn.commitIndex, rn.nextIndex, rn.matchIndex)
